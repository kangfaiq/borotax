...
<head>
    ...
    <title>SKPD Reklame - {{ $skpd->nomor_skpd }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="page">
        ...
        <table class="kop-table" style="margin-bottom: 0;">
            <tr>
                ...
                <td style="width: 20%; text-align: center; font-size: 9pt; vertical-align: middle;">
                    ...
                    {{ $skpd->masa_berlaku_mulai ? $skpd->masa_berlaku_mulai->format('d/m/Y') : 'dd/mm/yyyy' }}<br>
                    s.d.<br>
                    {{ $skpd->masa_berlaku_sampai ? $skpd->masa_berlaku_sampai->format('d/m/Y') : 'dd/mm/yyyy' }}
                </td>
            </tr>
            <tr>
        ...
        <table style="margin-bottom: 0; border-top: none;">
            <tr>
                ...
                <td style="border-top: none;">{{ $skpd->npwpd ?? $taxObject->npwpd ?? '-' }}</td>
            </tr>
            <tr>
        ...
        <table style="margin-bottom: 0; border-top: none;">
            <tr>
                <td rowspan="{{ $isSewaPemkab ? 4 : 6 }}" style="width: 12%; font-weight: bold; vertical-align: middle; text-align: center; border-top: none;">
                    Objek<br>Pajak</td>
                @if($isSewaPemkab)
                <td style="width: 22%; font-weight: bold; border-top: none;">Kode Aset</td>
                <td style="border-top: none;">{{ $asetPemkab->kode_aset ?? '-' }}</td>
                @else
            ...
            <tr>
                ...
                <td>{{ strtoupper($skpd->nama_reklame) }}</td>
            </tr>
            <tr>
                <td class="bold">Alamat Objek</td>
                <td>{{ $skpd->alamat_reklame }}</td>
            </tr>
            @if(!$isSewaPemkab)
            <tr>
                <td class="bold">Kelompok Strategis</td>
                <td>{{ $kelompokStrategis }}</td>
            </tr>
            @endif
            @if(!$isSewaPemkab)
            <tr>
            ...
            @endif
            @if($isSewaPemkab && $skpd->permohonanSewa?->jenis_reklame_dipasang)
            <tr>
                <td class="bold">Materi Terpasang</td>
                <td>{{ $skpd->permohonanSewa->jenis_reklame_dipasang }}</td>
            </tr>
            @endif
        </table>
        ...
        <table style="margin-bottom: 0; border-top: none;">
            ...
            @if($isSewaPemkab)
            {{-- Aset Pemkab: Layout khusus sewa --}}
            <tr>
            ...
            <tr>
                <td>&nbsp;&nbsp;&nbsp;&nbsp;a. Bentuk</td>
                <td class="right">{{ ucfirst($bentuk) }}</td>
            </tr>
            <tr>
                <td>&nbsp;&nbsp;&nbsp;&nbsp;b. Panjang (m)</td>
                <td class="right">{{ number_format($panjang, 2, ',', '.') }}</td>
            </tr>
            <tr>
                <td>&nbsp;&nbsp;&nbsp;&nbsp;c. Lebar (m)</td>
                <td class="right">{{ number_format($lebar, 2, ',', '.') }}</td>
            </tr>
            <tr>
                <td>&nbsp;&nbsp;&nbsp;&nbsp;d. Perhitungan</td>
                <td class="right">{{ $rumusLuas }}</td>
            </tr>
            <tr>
                <td>&nbsp;&nbsp;&nbsp;&nbsp;e. Luas (m²)</td>
            ...
            <tr>
                ...
                <td class="right">{{ match($skpd->satuan_waktu ?? 'perTahun') { 'perTahun' => 'Tahunan', 'perBulan' => 'Bulanan', 'perMinggu' => 'Mingguan', default => 'Tahunan' } }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="right">{{ number_format($tarifPokok, 0, ',', '.') }}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="right">{{ number_format(($nspr + $njopr) * $luasM2, 0, ',', '.') }}</td>
            </tr>
            <tr>
                <td>&nbsp;&nbsp;&nbsp;&nbsp;a. Bentuk</td>
                <td class="right">{{ ucfirst($bentuk) }}</td>
            ...
            <tr>
                <td>&nbsp;&nbsp;&nbsp;&nbsp;f. NSPR/({{ $satuanLabel }})</td>
                <td class="right">{{ number_format($nspr, 0, ',', '.') }}</td>
            </tr>
            <tr>
                <td>&nbsp;&nbsp;&nbsp;&nbsp;g. NJOPR/({{ $satuanLabel }})</td>
                <td class="right">{{ number_format($njopr, 0, ',', '.') }}</td>
            </tr>
            {{-- Tarif Pajak --}}
            <tr>
            ...
            <tr>
                ...
                <td class="right">{{ number_format($nilaiStrategis, 0, ',', '.') }}</td>
            </tr>
            <tr>
                ...
                <td style="width: 45%; text-align: center; font-size: 10pt;">
                    ...
                    @if($pimpinan)
                        @if(is_null($pimpinan->bidang) && is_null($pimpinan->sub_bidang))
                            {{-- 2. Pimpinan utama (bidang=null, sub_bidang=null): Jabatan + OPD --}}
                            {{ $pimpinan->jabatan }} {{ $pimpinan->opd }}<br>
                            {{ $pimpinan->kab }}
                        @elseif(is_null($pimpinan->sub_bidang))
                            {{-- 4a. sub_bidang=null: a.n. Jabatan OPD, Kab, Jabatan Bidang --}}
                            a.n. {{ $pimpinan->jabatan }} {{ $pimpinan->opd }}<br>
                            {{ $pimpinan->kab }}<br>
                            <span style="font-size: 9pt;">{{ $pimpinan->jabatan }} {{ $pimpinan->bidang }}</span>
                        @else
                            {{-- 4b. sub_bidang!=null: a.n. Jabatan OPD, Kab, Jabatan Sub_bidang, Bidang --}}
                            a.n. {{ $pimpinan->jabatan }} {{ $pimpinan->opd }}<br>
                            {{ $pimpinan->kab }}<br>
                    ...
                    <div style="margin: 12px auto; width: 80px; height: 80px;">
                        <img src="data:image/svg+xml;base64, {{ base64_encode((new \BaconQrCode\Writer(new \BaconQrCode\Renderer\ImageRenderer(new \BaconQrCode\Renderer\RendererStyle\RendererStyle(100), new \BaconQrCode\Renderer\Image\SvgImageBackEnd())))->writeString(route('skpd-reklame.show', $skpd->id))) }}"
                            alt="QR Code" style="width: 80px; height: 80px;">
                    </div>

                    @if($pimpinan)
                        {{-- 6. Nama (bold + underlined) --}}
